...
def main():
    ...
    print("Note: The DDS in Sim transmits messages on channel 1. Please ensure that other DDS instances use the same channel for message exchange by setting: ChannelFactoryInitialize(0, \"ens5\").")
    try:
        ...
        with contextlib.suppress(KeyboardInterrupt), torch.inference_mode():
            while simulation_app.is_running() and controller.is_running:
                ...
                LIDAR_PUBLISH_EVERY_N = 10  # with 100 Hz loop → ~10 Hz publish rate
                    ...
                    if loop_count % LIDAR_PUBLISH_EVERY_N == 0:
                        try:
                            ...
                            if point_cloud is not None:
                                pc_np = point_cloud.cpu().numpy()  # (N_rays, 3), may contain inf

                                # Filter out rays that missed (inf or nan)
                                valid_mask = np.isfinite(pc_np).all(axis=1)
